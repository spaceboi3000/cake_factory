...
export default function ConveyorBelt({ clockSpeed, powerHeat, isDead, onSetSpeed }: ConveyorBeltProps) {
  ...
  useEffect(() => {
    ...
    let baseRollerDeg = 0;
    const MOVE_FRACTION = 0.35; // 35% time advancing, 65% time stationary at station
    let animId: number;
    ...
    function render(now: number) {
      ...
      if (!isDeadRef.current) {
        ...
        // Step cadence directly tied to clock frequency (1.0 - 5.0 steps/sec)
        stepPhase += dt * speed;

        while (stepPhase >= 1.0) {
          stepPhase -= 1.0;

          // Advance grid positions for all cakes by one step
          for (let i = 0; i < cakesList.length; i++) {
            cakesList[i].gridX -= CAKE_SPACING;
          }

          // Offscreen recycling on the far left (x < -100px)
          for (let i = 0; i < cakesList.length; i++) {
            const c = cakesList[i];
            if (c.gridX < -100) {
              let maxGridX = -100;
        ...
        // Stepped motion: moves during [0, MOVE_FRACTION), then dwells stationary
        ...
        // Rotate rollers in sync with step motion
